...
#pragma config(Motor,  port9,           rightRoller,   tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port10,          leftRoller,    tmotorVex393_HBridge, openLoop, reversed)
...
void stackConeOnMogo(){
	int error = conePotVal - SensorValue[coneLift];
	motor[coneLeftLift] = 0.1 * error;
	motor[coneRightLift] = 0.1 * error;
	releaseCone();
}
void liftCone(){
	int error = (conePotVal+300) - SensorValue[coneLift];
	motor[coneLeftLift] = 0.1 * error;
	motor[coneRightLift] = 0.1 * error;
}
